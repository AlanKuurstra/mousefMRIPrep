...
def infotodict(seqinfo):
    ...
    TurboRARE_T2 = create_key('{bids_subject_session_dir}/anat/{bids_subject_session_prefix}_acq-TurboRARE_run-{item:02d}_T2w')

    rsFMRI = create_key('{bids_subject_session_dir}/func/{bids_subject_session_prefix}_task-rs_run-{item:02d}_bold')

    info = { FLASH_T1:[],FLASH_MT_ON:[],FLASH_MT_OFF:[],dwi:[],MP2RAGE_T1map:[],MP2RAGE_invs:[],MP2RAGE_UNI:[],MP2RAGE_T1map_l:[],MP2RAGE_invs_l:[],MP2RAGE_UNI_l:[],MEGRE_mag:[],MEGRE_complex:[],TurboRARE_T2:[],rsFMRI:[]}
    ...
    for idx, s in enumerate(seqinfo):
        ...
        if ('T1_FLASH' in s.protocol_name):
          ...
          if (s.dim1 > 64):
            ...
            info[TurboRARE_T2].append({'item': s.series_id})

        elif ('T2star_rsFMRI' in s.series_description.strip()):
            info[rsFMRI].append({'item': s.series_id})
...
def filter_dicom(dcm_data):
    skip_dicom = False
    skip_dicom = skip_dicom or dcm_data.SeriesDescription in [
        '1_Localizer_FOV80mm',
    ]
    return skip_dicom
