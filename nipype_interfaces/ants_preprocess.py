...
from nipype.interfaces.base import (BaseInterface, BaseInterfaceInputSpec, TraitedSpec, File, traits, isdefined, Str)
...
# def get_time_dimensions(nifti_filename):
#     header = nib.load(nifti_filename).get_header()
#     nvolumes = header.get_header()['dim'][4]
#     tr=header.get_header()['pixdim'][4]
#     return nvolumes,tr

# get_time_info = pe.Node(
#     Function(input_names=["input_file"], output_names=["nvolumes","tr"], function=get_time_dimensions),
#     name="get_time_info")

# class ReplicateDisplacementInputSpec(CommandLineInputSpec):
#     input_transform = File(desc="File", exists=True, mandatory=True, argstr="%s")
#     tr = traits.Float(1.0, desc='TR of bold scan', mandatory=True, argstr='%s')
#     nvolumes = traits.Int(1, desc='Number bold volumes', mandatory=True, argstr='%s')


# class ReplicateDisplacement(CommandLine):
...
class ReplicateImageInputSpec(CommandLineInputSpec):
    input_3d_image = File(desc="File", exists=True, mandatory=True, argstr="%s")
    bold_file = File(desc="File", exists=True, mandatory=True, argstr="%s")
    output_file = File('replicated_image.nii.gz',desc="File", mandatory=False, argstr="%s", usedefault=True)
...
class ReplicateImage(BaseInterface):
    ...
    def _run_interface(self, runtime):
        ...
        input_bold_image_obj = nib.load(self.inputs.bold_file)
        header = input_bold_image_obj.get_header()
        nvolumes = str(header['dim'][4])
        tr = str(header['pixdim'][4])
        output_filename = self.inputs.output_file

        # process inputs
        print(" ".join(['ImageMath','3',output_filename,'ReplicateImage',input_3d_image,nvolumes,tr,'0']))
        subprocess.call(['ImageMath','3',output_filename,'ReplicateImage',input_3d_image,nvolumes,tr,'0'])
...
class ReplicateDisplacementInputSpec(CommandLineInputSpec):
    input_transform = File(desc="File", exists=True, mandatory=True, argstr="%s")
    bold_file = File(desc="File", exists=True, mandatory=True, argstr="%s")
    output_file = File('replicated_transform.nii.gz',desc="File", mandatory=False, argstr="%s", usedefault=True)
...
class ReplicateDisplacement(BaseInterface):
    ...
    def _run_interface(self, runtime):
        ...
        input_image_obj = nib.load(self.inputs.bold_file)
        header = input_image_obj.get_header()
        nvolumes = str(header['dim'][4])
        tr = str(header['pixdim'][4])
        output_filename = self.inputs.output_file

        # process inputs
        print(" ".join(['ImageMath','3',output_filename,'ReplicateDisplacement',input_transform,nvolumes,tr,'0']))
        subprocess.call(['ImageMath','3',output_filename,'ReplicateDisplacement',input_transform,nvolumes,tr,'0'])
